...
func RenderView(screen screen.Screen) {
	...
	drawWorld(screen)
	drawUI(screen)
	renderToScreen(screen, fmt.Sprintf("FPS: %6.2f; POV:%4.2f; FOV:%4.2f, player_pos:(X:%9.4f,Y:%9.4f)", 1/TimeElapsed, player.POV.Angle(), player.FOV, player.X, player.Y))
...
func drawWorld(screen screen.Screen) {
	leftVector := Vector{X: -player.POV.Y, Y: player.POV.X} // leftVector is current player's dir vector rotated 90 degrees left
	...
	for i := 0; i <= screen.Width(); i++ {
		...
		stepX, stepY := player.POV.X+progress*(leftVector.X*2), player.POV.Y+progress*(leftVector.Y*2)
		curVector := Vector{X: stepX, Y: stepY}

		hit, distanceToWall, tile, tileP := rayCast(player.X, player.Y, curVector, player.ViewDist)
